...
class Model_Adam(Model_RMSPROP, Model_Momentum):
    def __init__(self, X, y, n_iter, eta=0.05, batch_size=4, b1 = 0.25, b2 = 0.9, eps = 1e-8):
    ...
    def fit(self):
        ...
        for i in range(self.n_iter):
            ...
            self.G = (self.betta * self.G + (1-self.betta)*(self.compute_grad(self.X[ind, :], self.y[ind], self.w)**2))/(1-math.pow(self.betta, i+1))
            self.nu = (self.alpha * self.nu + (1-self.alpha) * self.compute_grad(self.X[ind, :], self.y[ind], self.w))/(1-math.pow(self.alpha, i+1))
